...
MAX_PAYBACK_DAYS = float(os.getenv("MAX_PAYBACK_DAYS", "8"))
STRONG_SIGNAL_PAYBACK_DAYS = float(os.getenv("STRONG_SIGNAL_PAYBACK_DAYS", "3"))
...
class RadarDB:
    ...
    def ensure_scan_result_columns(self):
        ...
        with self.conn() as con:
            existing = {r[1] for r in con.execute("PRAGMA table_info(scan_results)").fetchall()}
            for col, col_type in columns.items():
                if col not in existing:
                    logger.info(f"DB migrate: add scan_results.{col}")
...
class Http:
    ...
    async def post_signed(self, base: str, path: str, params: Dict[str, Any]):
        ...
        async with self.sem:
            async with self.session.post(base + path, params=params, headers=headers) as resp:
                text = await resp.text()
...
def estimate_buy_slippage(book: Dict[str, Any], notional: float) -> Optional[float]:
    ...
    return max(0.0, (avg_price - mid) / mid)
...
def estimate_sell_slippage(book: Dict[str, Any], notional: float) -> Optional[float]:
    ...
    avg_price = proceeds_sum / qty_sum
    return max(0.0, (mid - avg_price) / mid)
...
def analyze_history(history: List[Dict[str, Any]], current_rate: float) -> Tuple[Optional[Dict[str, Any]], str]:
    ...
    if pos_ratio < POSITIVE_RATIO_THRESHOLD:
        reasons.append(f"正費率比例不足：{pos_ratio:.2f}")

    if avg < AVG_FUNDING_RATE_THRESHOLD:
        reasons.append(f"7日平均不足：{fmt_pct(avg)}")

    if std > MAX_FUNDING_STD_7D:
        reasons.append(f"標準差過高：{fmt_pct(std)}")

    if avg > 0 and (std / avg) > MAX_STD_TO_AVG_RATIO:
        reasons.append(f"標準差相對平均過高：{std / avg:.2f}")

    if max_abs > MAX_ABS_HISTORICAL_FUNDING_RATE:
        reasons.append(f"歷史異常值過高：{fmt_pct(max_abs)}")

    if recent_avg < avg * RECENT_AVG_MIN_RATIO_TO_7D:
        reasons.append("近期費率衰退")

    if current_rate < avg * CURRENT_MIN_RATIO_TO_7D:
        reasons.append("當前費率低於7日平均過多")

    return stats, "；".join(reasons)
...
def calc_entry_cost_rate(spot_slip: float, fut_slip: float) -> float:
    ...
    slippage_rate = abs(safe_float(spot_slip, 0.0)) + abs(safe_float(fut_slip, 0.0))
...
def calc_roundtrip_cost_rate(spot_slip: float, fut_slip: float) -> float:
    ...
    return entry_cost * 2 if INCLUDE_EXIT_COST else entry_cost
...
def classify(payback: float, net_apy: Optional[float]) -> str:
...
class Telegram:
    ...
    def target_net_text(self, net_apy) -> str:
        ...
        try:
            gap = float(net_apy) - TARGET_NET_APY
    ...
    async def cmd_status(self):
        ...
        with self.db.conn() as con:
            ...
            latest_ts = con.execute("SELECT MAX(ts) FROM scan_results").fetchone()[0]
        ...
        await self.send(
            ...
            f"固定追蹤：<code>{self.h(','.join(sorted(ALWAYS_TRACK_SYMBOLS)))}</code>\n\n"
            f"資料庫：<code>{self.h(DB_PATH)}</code>\n"
    ...
    async def cmd_why(self, symbol: str):
        ...
        if not row:
            await self.send(
                ...
                "查無這個交易對的掃描資料。\n\n"
                "如果你希望固定追蹤，請把它加入：\n"
                "<code>ALWAYS_TRACK_SYMBOLS</code>"
            )
            return
        ...
        if status == "PASS":
            explanations.append("此交易對目前通過正向套利真實淨利篩選。")
        elif status == "WATCH":
            explanations.append("此交易對目前列為觀察，通常代表當前 funding 偏高，但風險條件尚未完全通過。")
        elif status == "FAIL":
            explanations.append("此交易對目前未通過正向套利篩選。")
        else:
            explanations.append("此交易對目前狀態不明。")

        if fail_reason:
            explanations.append(f"未通過原因：{fail_reason}")
        ...
        msg = (
            ...
            f"當前資金費率：{fmt_pct(self.row_get(row, 'current_funding_rate'))}\n"
            f"7日平均資金費率：{fmt_pct(self.row_get(row, 'avg_funding_rate_7d'))}\n"
            ...
            f"合約 OI 名目價值：<code>{self.fmt_money(self.row_get(row, 'open_interest_notional'))}</code>\n\n"
            "📌 <b>診斷說明</b>\n"
            + "\n".join([f"- {self.h(x)}" for x in explanations])
        )
    ...
    async def analyze(self, c: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if net_apy is None or net_apy < TARGET_NET_APY:
                fail_reasons.append(f"扣除成本後真實淨年化不足：{safe_float(net_apy, 0.0) * 100:.2f}%")
